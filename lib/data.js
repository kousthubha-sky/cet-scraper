...
function db() {
  ...
  _cache = {
    ...
    students: loadOptional("students.json"),
  };
  return _cache;
}
...
/** Look up a student by KCET application number (case-insensitive). */
export function getStudent(cetNumber) {
  if (!cetNumber) return null;
  const key = String(cetNumber).trim().toUpperCase();
  return db().students.find((s) => s.cetNumber.toUpperCase() === key) || null;
}
